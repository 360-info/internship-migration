...
The purpose of this visualisation is to offer an immediately view of the number of dead and missing migrants overtime and across regions. Therefore, a heatmap  \@ref(fig:heatmapv1) is created at the first instance. Audiences could easily capture the region with darkest colour to be the most dangerous region that many migrants lost their lives in. However, a single heatmap is not enough as it does not have the ability to tell audience the number changing overtime. Therefore, two marginal plots are created in corporate with the heatmap as shown \@ref(fig:heatmapv2), that fulfills the requirement of offering the audience a brief overview of the situation overtime and across  regions.
...
```{r heatmapv1, fig.cap="Heat map version 1"}
mhmv1 <- mg1 %>%
  mutate(`Incident year` = as.character(`Incident year`)) %>%
...
```{r heatmapv2, fig.cap="Heat map version2"}
mhm1 <- mg1 %>%
  mutate(`Incident year` = as.character(`Incident year`)) %>%
  ggplot(aes(x = `Incident year`,
             y = `Region of Incident`,
...
Human trafficking routes are visualised using the Sankey diagram, the aim of this visualisation is to explore the routes that most victims are trafficked to. At the first stage of creating the visualisation, an interactive html widget package called `parsetR` is attempted, however, this html widget has a lot of restriction in customizing the visualisation \@ref(fig:routesv1). The title and variable names cannot be modified, furthermore, it is not allowed to add 360 theme to align with the 360info’s style of visualisation. Therefore, `gg_alluvium` and `gg_stratum` is suggested by [James](https://github.com/jimjam-slam) to visualise the human trafficking routes, with opacity set to indicate the number of victims transported in each trafficking route \@ref(fig:routesv2).
...
```{r routesv1, fig.cap="Routes version 1"}
...
```{r featurefinal, fig.cap="Victim Feature final version"}
...
A stacked bar chart \@ref(fig:dfeature) is applied to demonstrate the demographic distribution of victims of all reported human trafficking cases from 2002 to 2019. Not surprisingly, Female adult are the main group of victims across most type of exploits recorded by the dataset. Sexual exploitation is the largest category of exploitation with 15989 reported cases fall under this category, followed by forced labour.  
...
```{r dfeature, echo=FALSE, out.width="80%", fig.cap="Victim Distribution"}
...
For future analysis, it is expected that more incidents and cases can be found and recorded by increasing speed of information dissemination, hence contribute to efforts to better understand the causes of incidents, population that are more likely to suffer. 
